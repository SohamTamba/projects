...
4. **Adversarial perturbation transformation makes classification significantly more difficult for a neural network**
...
However, the improvement in performance was not as significant as were hoping for; Perhaps we should focus more on developing better pretext tasks, rather than techniques to boost pre-existing pretext tasks. 

___
___

**Update on 20th July 2020:** It has been found that adversarial noise improves the feature extractor obtained from pretraining a model on ImageNet. Coincidentally, the findings were published by the professor I mentioned in the logic section. The work can be found [here](https://gradientscience.org/transfer-learning/).
