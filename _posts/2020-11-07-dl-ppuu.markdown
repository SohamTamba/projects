...
class ThresholdedWorldModel(nn.Module):
    ...
    def forward(self, current_state, action):
        ...
        if self.policy_training:
            next_state = Threshold.apply(next_state)
        return next_state
...
\begin{cases}
-y\ \ \ \ \ \ \ \ \ \text{if}\ y \leq 0.1 \\
1-y\ \ \ \ \ \text{else if}\ y \geq 0.9 \\
0\ \ \ \ \ \ \ \ \ \ \ \ \text{else}
\end{cases}
$$

Applying the stop gradient to the $\epsilon$ term  may lead to an issue in backpropagation, but it might be justified by the fact that its gradient has a absolute value of atmost $0.09$, and setting $0.09$ to $0$ may not have much effect.

<br>

I trained two agents, using the same hyper-parameters and training epochs; one with the thresholding scheme and one without. I then evaluated their performance using the benchmark mentioned above.

I also evaulated the performance of the two models using test images that were downsampled using nearest mode instead of billinear.
...
Note that the performance of an agent varies wildly with hyper-parameters, random seed and training steps. The hyper-parameters are tuned for training the agents using a regular World Model.
...
**As you can see from the above table, the agent trained using a Thresholded World Model outperforms the one trained using a regular World Model 11 out of 15 times has better average performance, best performance and variance.** This seems like a significant improvement, but it is worth noting that the performance of an agent varies wildly with hyper-parameters, random seed and training steps.

**More importantly, the agent achieves a success rate of 77.2% without any hyper-parameter tuning and using a single random seed. In comparison, regarding models trained without this technique, 10 agents were trained using a different random seed and only 3 of them achieved a success rate of 77%, after significant hyper-parameter tuning.** For more details on the 10 agents, please refer to the [`evaluation notebook`](https://github.com/Atcold/pytorch-PPUU/blob/master/Notebooks/Plotting-success-rate.ipynb).
...
Thresholding the output of the World Model - which effectively modifies the final activation function - improved the performance of the driving agent, but it is worth noting that the performance of an agent varies wildly with the training steps, hyperparameters and random seed, so it is difficult to judge its statistical significance.
