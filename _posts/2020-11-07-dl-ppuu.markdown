...
As expected, the performance is very poor, with a peak survival rate of 0.6%. The median distance traveled is approximately 200m, which is the length of a highway stretch. This indicates that a majority of cars crash before the transition from one highway to another is complete.

<br>

I then conducted the benchmark while setting the safe zone to the portion of the highway that is visible to the ego car.
